...
if __name__ == '__main__':
    b_list = [1] # [1, 16, 100]
    size_list = [2048] # [128 * i for i in range(10, 40)]
                        ...
                        for group_m in group_m_list:
                            for num_stage in num_stage_list:
                                for num_warps in num_warps_list:
                                    print(f"b: {b}, size: {size}, block_m: {block_m}, block_n: {block_n}, block_k: {block_k}, group_m: {group_m}, num_stage: {num_stage}, num_warps: {num_warps}")
                                    ...
                                    os.system(f"/opt/nvidia/nsight-compute/2023.1.0/ncu \
                                              ...
                                              --csv python3 03-2-batched-matrix-multiplication-ncu-profiling.py --b {b} --m {size} --n {size} --k {size} --block-m {block_m} --block-n {block_n} --block-k {block_k} --group-m {group_m} --num-stages {num_stage} --num-warps {num_warps} | \
                                                tee csv/nsight-compute-03-2-batched-matrix-multiplication-ncu-profiling-{b}_{size}_{size}_{size}-{block_m}_{block_n}_{block_k}_{group_m}-{num_stage}_{num_warps}.csv")